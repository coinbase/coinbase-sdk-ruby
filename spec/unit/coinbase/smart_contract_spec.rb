...
describe Coinbase::SmartContract do
  ...
  let(:from_block_height) { 201782330 }
  let(:to_block_height) { 201782340 }
  ...
  before do
    ...
    allow(contract_events_api).to receive(:list_contract_events).and_return(
      instance_double(Coinbase::Client::ListContractEvents200Response, data: [contract_event_model],
                                                                       has_more: true,
                                                                       next_page: 'next_page'),
      instance_double(Coinbase::Client::ListContractEvents200Response, data: [], has_more: false)
    )
  end
  ...
  describe '.list_events' do
    subject(:list_events) do
      described_class.list_events(
        network_id, protocol_name, contract_address, contract_name, event_name, from_block_height, to_block_height
      )
    end

    it 'fetches the first page of contract events' do
      list_events.to_a
    ...
    it 'fetches the last page of contract events' do
      list_events.to_a
